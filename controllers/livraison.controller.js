...
const GetFarmerOrder = async (req, res) => {
  try {
    ...
    const orderFarmer = await orderModel.find({ products: { $in: productsId }, etat: false });
...
const Payment = async (req, res) => {
  ...
  const payload = {
    "app_token": "37d31519-6999-4caf-81b4-d4f8a3dbcddf",
    "app_secret": "908d5f03-16b4-41f7-8b24-c68c92837e61",
    "amount": req.body.amount,
    "accept_card": "true",
    "session_timeout_secs": 1200,
    "success_link": "http://localhost:3000/success",
    "fail_link": "http://localhost:3000/failed",
    "developer_tracking_id": "47e039a9-17b7-4144-be98-a9e13de3a67f"  
  }
...
const Deliver = async (req, res) => {
  try {
    const order = await orderModel.findByIdAndUpdate(req.params.order, { etat: true }, { new: true })
    res.status(200).json(order);
  } catch (error) {
    res.status(404).json(error.message);
  }
...
module.exports = {
  GetUserLivraison, PlaceOrder, Payment, Verify, updateOrder, GetOrder, cancelOrder, GetFarmerOrder, GetLivraisonByReference, Deliver
};
